...
string* filtr(int n, int m, string** a, int choice4)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            if (k == 1)
                cout << "Подробная информация о " << choice4 << " студенте: " << endl << "Номер группы " << a[i][0] << ", " << a[i][2] << ", оценка: " << a[i][3] << ", название дисциплины: " << a[i][4] << endl;
            else
...
string* deleting(int n, int m, string** a, int choice9)
{
    ...
    if (choice9 + 1 > n || choice9 <= 0) 
        cout << "Данного студента найти не удалось. Повторите попытку позже." << endl;
    if (choice9 + 1 == n)
    {
        for (int i = 0; i < n; i++)
        {
...
int main()
{
    ...
    while (choice != 0)
    {
        ...
        {
            int choice3;
            cout << endl << "У кого студента требуется отредактировать данные? ";
            cin >> choice3;
            cin.ignore();
        ...
        if (choice == 7)
        {
            ...
            if (choice7 == 2)
            {
                cout << endl << "Таблица дисциплин в порядке возрастания среднего балла: " << endl << endl << "№ | ФИО |   Номер группы   | Оценка | Дисциплина " << endl; 
