...
string* filtr(int n, int m, string** a, int choice4)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            if (k == 1)
                cout << "Подробная информация о " << choice4 << " студенте: " << endl << "ФИО: " << a[i][0] << ", " << a[i][2] << ", оценка: " << a[i][3] << ", название дисциплины: " << a[i][4] << endl;
            else
                cout << "ФИО студента: " << a[i][0] << ", " << a[i][2] << ", оценка: " << a[i][3] << ", название дисциплины: " << a[i][4] << endl;
            k++;
            prove = 1;
        }
    }
    if (prove == 0)
        cout << "К сожалению, студента с таким ФИО в этой группе найти не удалось. Повторите попытку позже." << endl;
    return 0;
}
...
string* sredniybal(int n, int m, string** a)
{
    ...
    {
        if (b[i][1] == max)
            cout << "название дисциплины " << b[i][0] << " (средний балл = " << max << ")" << endl;
    }
    return 0;
}
...
{
    int str;
    cout << endl << "Подробная информация о студенте из группы " << nom << ": " ;
    for (int i = 0; i < n; i++)
    {
        if (stoi(a[i][0]) == nom)
            str = i;
    }
    cout << "ФИО " << a[str][1] << ", " << a[str][2] << ", оценка: " << a[str][3] << ", название дисциплины: " << a[str][4] << endl;
    return 0;
}
...
string* sortirovka2(int n, int m, string** spisok)
{
    ...
    for (int k = 0; k < n; k++)
    {
        for (int i = 0; i < n - 1; i++)
        {
            znach = stoi(spisok[i + 1][3]);
            if (stoi(spisok[i][3]) > znach)
            {
                for (int j = 0; j < m; j++)
                {
                    string save;
...
string* deleting(int n, int m, string** a, int choice9)
{
    ...
    if (choice9 + 1 > n || choice9 <= 0) 
        cout << "Такой номер группы найти не удалось. Повторите попытку позже." << endl;
    if (choice9 + 1 == n)
    {
        for (int i = 0; i < n; i++)
        {
...
{
    int summa = 0;
    cout << "Средний балл по дисциплинам: ";
    for (int i = 0; i < n; i++)
    {
...
int main()
{
    cout << "Добро пожаловать в приложение!" << endl << "Самое время выставить оценки студентам! :) " << endl << endl << "Введите количество студентов: "; 
    int n;
    cin >> n;
    cin.ignore();
    int m = 5; 
    ...
    while (choice != 0)
    {
        if (choice == 1)
        {
            cout << endl << "Введите новое количество студентов, не превышающее " << n << ": ";
            int student;
            cin >> student;
            cin.ignore();
            cout << endl;
        ...
        {
            int choice3;
            cout << endl << "У студента под каким номером требуется внести изменения? ";
            cin >> choice3;
            cin.ignore();
        ...
        if (choice == 6)
        {
            cout << endl << "Студента под каким номером Вы хотите посмотреть? ";
            int nom;
            cin >> nom;
            if (nom > n)
                cout << endl << "Такой студент в таблице не найден. Повторите попытку позже. " << endl;
            else
                nomer(n, m, spisok, nom);
        }
        if (choice == 7)
        {
            cout << "Выберите сортировку: " << endl << "1 - в прямом алфавитном порядке " << endl << "2 - в обратном алфавитном порядке " << endl;
            int choice7;
            cin >> choice7;
            if (choice7 == 1)
            {
                cout << endl << "ФИО студентов в прямом алфавитном порядке: " << endl << endl << "№ |         ФИО            |   Номер группы   |  Оценка | Название дисциплины " << endl; 
                sortirovka1(n, m, spisok);
            }
            if (choice7 == 2)
            {
                cout << endl << "ФИО студентов в обратном алфавитном порядке: " << endl << endl << "№ |         ФИО            |   Номер группы   |  Оценка | Название дисциплины " << endl; 
        ...
        {
            z = n + 1;
            cout << "Заполните информацию о новом студенте: " << endl << endl;
            cin.ignore();
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < m; j++)
        ...
        if (choice == 9)
        {
            cout << endl << "Студента под каким номером вы хотите удалить? " << endl;
            int choice9;
            cin >> choice9;
    ...
    cout << "Продуктивно! Спасибо, что воспользовались нашим приложением! До новых встреч! ";
    return 0;
}
